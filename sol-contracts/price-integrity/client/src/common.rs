...
use tickx_price_integrity_sol::{
    instruction::{PriceIntegrityInstruction, METRIC_FEED_COUNT},
    state::{CONFIG_SEED, REPORT_SEED},
};
...
pub fn get_optional_flag_value(args: &[String], flag: &str) -> Option<String> {
    let mut iter = args.iter();
    while let Some(arg) = iter.next() {
        if arg == flag {
...
pub fn report_pda(program_id: &Pubkey, epoch_id: u64) -> (Pubkey, u8) {
    Pubkey::find_program_address(&[REPORT_SEED, &epoch_id.to_le_bytes()], program_id)
}

pub fn parse_feed_ids_csv(input: &str) -> Result<[[u8; 32]; METRIC_FEED_COUNT], Box<dyn Error>> {
    let parts: Vec<&str> = input.split(',').map(str::trim).filter(|s| !s.is_empty()).collect();
    if parts.len() != METRIC_FEED_COUNT {
        return Err(format!(
    ...
    let mut feed_ids = [[0_u8; 32]; METRIC_FEED_COUNT];
...
pub fn parse_hash_32(args: &[String], flag: &str) -> Result<[u8; 32], Box<dyn Error>> {
    parse_hex_32(&get_flag_value(args, flag)?)
}

pub fn derive_quote_account(queue: &Pubkey, feed_ids: &[[u8; 32]; METRIC_FEED_COUNT]) -> Pubkey {
    let mut seeds: Vec<&[u8]> = Vec::with_capacity(1 + METRIC_FEED_COUNT);
    seeds.push(queue.as_ref());
    for feed_id in feed_ids {
        seeds.push(feed_id.as_slice());
    }
    let quote_program_id =
        Pubkey::from_str(QUOTE_PROGRAM_ID_STR).expect("valid switchboard quote program id");
    Pubkey::find_program_address(&seeds, &quote_program_id).0
}

pub fn resolve_quote_account(
    args: &[String],
    queue: &Pubkey,
    feed_ids: Option<&[[u8; 32]; METRIC_FEED_COUNT]>,
) -> Result<Pubkey, Box<dyn Error>> {
    if let Some(value) = get_optional_flag_value(args, "--quote-account") {
        return Ok(Pubkey::from_str(&value)?);
    }

    let feed_ids = feed_ids.ok_or("--quote-account or --feed-ids is required")?;
    Ok(derive_quote_account(queue, feed_ids))
}

pub fn build_instruction(
    program_id: Pubkey,
    instruction: PriceIntegrityInstruction,
...
pub fn set_switchboard_config_accounts(owner: &Pubkey, config: &Pubkey) -> Vec<AccountMeta> {
    vec![AccountMeta::new(*owner, true), AccountMeta::new(*config, false)]
}
