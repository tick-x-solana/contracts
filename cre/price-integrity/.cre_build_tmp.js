...
var computeMetrics = (internalCandles, chainlinkCandles) => {
  ...
  errors2.sort((a, b) => a - b);
  const mae = errors2.reduce((a, b) => a + b, 0) / errors2.length;
  const max = errors2[errors2.length - 1];
  const p95Index = Math.floor(errors2.length * 0.95);
  const p95 = errors2[Math.min(p95Index, errors2.length - 1)];
...
var computeScore = (metrics, candleCount) => {
  ...
  return Math.floor((weightAccuracy * sAcc + weightP95 * sP95 + weightMax * sMax + weightDirection * sDir + weightOutliers * sOut) / 1e4);
};
var fetchOhlcCandles = (runtime2, apiKey, windowStart, windowEnd, source) => {
  const client = new ClientCapability2;
  const url = `${runtime2.config.appApiBaseUrl}/ohlc?windowStart=${windowStart}&windowEnd=${windowEnd}&source=${source}`;
...
var onCronTrigger = (runtime2, payload) => {
  ...
  let apiKey = "";
  try {
    const secret = runtime2.getSecret({ id: "APP_API_KEY" }).result();
    apiKey = secret.value;
    runtime2.log(`API key loaded from secrets: ${apiKey.substring(0, 4)}...`);
  } catch (e) {
    const envKey = runtime2.secrets?.APP_API_KEY;
    if (envKey) {
      apiKey = envKey;
      runtime2.log(`API key loaded from env: ${apiKey.substring(0, 4)}...`);
    } else {
      apiKey = "8d920faa0c";
      runtime2.log(`API key using fallback: ${apiKey.substring(0, 4)}...`);
    }
  }
  ...
  runtime2.log(`MAE: ${metrics.ohlcMaeBps} bps, P95: ${metrics.ohlcP95Bps} bps, Max: ${metrics.ohlcMaxBps} bps`);
  runtime2.log(`Direction Match: ${metrics.directionMatchBps / 100}%, Outliers: ${metrics.outlierCount}`);
  ...
  if (metrics.ohlcP95Bps > priceIntegrityConfig.maxOhlcP95Bps)
    failureFlags |= 2;
  runtime2.log(`Passed: ${isPassed}, Failure Flags: ${failureFlags}`);
  ...
  const txHash = submitPriceIntegrityReport(runtime2, evmConfig, {
    epochId,
    windowStart,
